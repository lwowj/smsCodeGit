        ...
        function (exports) {
            ...
            self.tab = {
                ...
                init: function () {
                    ...
                    layui.dropdown.render({
                        ...
                        click: function (name) {
                            ...
                            if (name === 'current') {
                                //sw
                                if(layui.febs.route.fileurl != conf.entry){
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('febs-tabs-close')) {
                            ...
                            if (type === 'page') {
                                ...
                                if( conf.loadedIndex){ //sw说明首次刷新的非首页
                                    self.tab.data.push({
                                        fileurl:conf.entry,
                                        href:conf.entry
                                    })
                                    ...
                                    //点击页签首次加载首页
                                    conf.loadedIndex = true;  //sw首页已加载
                                    conf.isChangeHash = true; //sw首页页签点击首次加载需改变hash
                                }
                                tab.change({
                                    fileurl:conf.entry,
                                    href:conf.entry
                ...
                refresh: function (url) {
                    ...
                    //sw
                    for(var i=self.tab.data.length-1; i>=0; i--) {
                        if (self.tab.data[i].fileurl == conf.entry) {
                            self.tab.data.splice(i, 1);
                        }
                    }
                    if (this.has(url) || url == conf.entry) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                },
                ...
                change: function (route, callback) {
                    ...
                    if (existsTab) {
                        ...
                        //sw 解决非正常页面(/index#XXX)显示问题
                        //todo 解决地址栏hash变化问题?
                        if(!layui.router().href || layui.router().href === '/'){
                            $(".febs-tabs-item[lay-url='"+conf.entry+"']").remove();
                        }
                        //sw 判断访问不存在的页面地址
                        self.loadHtml(fileurl, function (res) {
                            ...
                            tab.data.push(route);
                            //sw
                            for(var i=tab.data.length-1; i>=0; i--) {
                                if (tab.data[i].fileurl == conf.entry) {
                                    tab.data.splice(i, 1);
                                }
                            }
                            //渲染出页面切换标签
                        ...
                        //sw首页页签点击首次加载需改变hash
                        if( conf.isChangeHash){
                            self.tab.data.push({
                                fileurl:conf.entry,
                                href:conf.entry
                            })
                            conf.isChangeHash = false;
                            layui.febs.navigate(route.href)
                        }
                    }
