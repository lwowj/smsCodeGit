...
layui.define(['jquery'], function (exports) {
    ...
    exports('validate', {
        ...
        orgAccount: function (value, item) {
            debugger
            if (!isEmpty(value)) {
                var result = '';
                $.ajax({
                    url: ctx + 'organizationUser/check/' + value,
                    data: {
                        "userId": item.getAttribute('id')
                    },
                    async: false,
        ...
        domain: function (value) {
            if (!isEmpty(value)) {
                if (!new RegExp("^(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$").test(value)) {
                    return '请填写正确的域名';
                }
            }
        },
