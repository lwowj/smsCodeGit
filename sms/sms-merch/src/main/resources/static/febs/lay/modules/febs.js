layui.extend({
    ...
    //sw
    if(layui.router().path.length > 0){ //说明首次刷新的非首页
        if(layui.router().href != '/'){
            layui.conf.loadedIndex = false;
        }
    }
    ...
    self.popup = function (params) {
        ...
        params.success = function(layero, index){
            view.loadHtml(conf.views + url, function (res) {
                ...
                view.parse(layero);  //解析页面可能存在的tpl
                $(layero).find('.layui-layer-content').html(htmlElem.html());
            });
        };
        var index = layer.open($.extend(defaultParams, params));
        //当小于这个尺寸的时候会进行手机端弹窗适配
    ...
    // ajax post请求
    self.post = function (url, params, success, fail) {
        if (params) {
            params.invalidate_ie_cache = new Date();
        }
    	//打开遮罩层
        ...
        $.post(url, params, function (r) {
            resolveResponse(r, success, fail);
        })
    };
    ...
    self.download = function (url, params, fileName) {
        ...
        xhr.onload = function () {
            ...
            } else if(this.status == 516){//  大小超过限制
                self.view.loadBar.error();
                var exportLimit = this.getResponseHeader("exportLimit");
                layer.alert("导出文件超过最大记录数【" + exportLimit +"】限制，下载失败.请分批导出！");
            }else {
                self.view.loadBar.error();
    ...
    function resolveResponse(r, f, s) {
    	//关闭遮罩层
        layer.close(layerIndex);
        if (r.code === 200) {
            f(r) && (f)();
    ...
    self.isEmojiCharacter=function(substring){
        if(substring){
