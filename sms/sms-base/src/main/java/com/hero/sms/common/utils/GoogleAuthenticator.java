 ...
 * @author yigehaoren
 *
 * @version 创建时间：2019年4月1日 18:08:00
 *
 *
 */
public class GoogleAuthenticator {
    ...
    public static final String SEED = "o876v4UxR1ENQTQJQARbEgj0tCNKVm74LVb4KhMOMFCxt0VhRGFYVqSjbXPHswsiubWUpBymibkX";
    // Java实现随机数算法。 在linux默认使用 /dev/random 生成随机种子，生成速度很慢，频繁调用会阻塞
    // 解决办法  启动项目指定使用 /dev/urandom ==> 使用启动命令： java -jar -Djava.security.egd=file:/dev/./urandom XXX.jar
    public static final String RANDOM_NUMBER_ALGORITHM = "SHA1PRNG";
    /**
         * 谷歌验证码在linux中阻塞问题补充。
         * 若使用SHA1PRNG算法，在linux中需使用启动命令java -jar -Djava.security.egd=file:/dev/./urandom XXX.jar
         * 若算法修改成NativePRNGNonBlocking:SUN.则只要默认命令启动即可，但windows中不支持该算法。
    */
    //NativePRNGNonBlocking:SUN算法。linux中 用/dev/urandom作为熵池，修复阻塞问题.但windows不支持
    public static final String RANDOM_NUMBER_NativePRNGNonBlocking = "NativePRNGNonBlocking";
    // 最多可偏移的时间
    int window_size = 2; // default 3 - max 17
    ...
    public static String generateSecretKey() {
        ...
        try {
            sr = SecureRandom.getInstance(RANDOM_NUMBER_NativePRNGNonBlocking);
    ...
    public static boolean verifyGoogleKey(String googleCode,String googleKey) {
    	long code = Long.valueOf(googleCode);
        long t = System.currentTimeMillis();
    	GoogleAuthenticator ga = new GoogleAuthenticator();
        ga.setWindowSize(5);
        
        return ga.check_code(googleKey, code, t);
    }
    /**
